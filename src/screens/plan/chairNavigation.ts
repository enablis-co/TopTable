 ...
 * DOM, so it is testable directly. Backs the roving-tabindex contract: exactly one tab stop per
 * table, arrows moving between chairs, each table remembering the chair it was left on.
 */
 ...
 * to the first, `End` to the last — wrapping at both ends. Any other key returns `null` so the
 * caller knows not to `preventDefault` it — a chair is one of several focusable things on the
...
/**
 * "Seat 3, Danny Whitaker" or "Seat 3, empty" — 1-based, matching both the table detail panel's
 * own rows and KB-4's printed seat positions.
 */
